...
export default function SocialInspiredHome({
  ...
  return (
    <main className="social-home">
      ...
      <section id="collection" className="social-collection">
        <div className="social-section-head">
          <div>
            <p className="social-micro">02 / THE COLLECTION</p>
            <h2>
              PICK YOUR
              <br />
              KIND OF REST.
          ...
          <p className="social-section-intro">
            Each mattress has a clear job. Scroll through the collection
            and choose by problem, not jargon.
          </p>
        </div>

        <div className="social-product-stack">
          {featured.map((product, index) => (
            <article
              key={product.slug}
              className="social-product-panel"
              style={{
                top: `calc(6.7rem + ${index * 1.3}rem)`,
                zIndex: index + 1,
              }}
            >
              <div className="social-product-visual">
                <img
                  src={product.image}
                  alt={product.name}
                  loading={index === 0 ? "eager" : "lazy"}
                />
                <span className="social-product-index">
                  0{index + 1}
                </span>
              </div>
              ...
              <div className="social-product-copy">
                ...
                <h3>{product.name}</h3>
                <p className="social-product-kicker">{product.kicker}</p>
                <p>{product.shortDescription}</p>

                <div className="social-product-meta">
                  <span>{product.firmness}</span>
                  <span>{product.trial}</span>
                  <span>{product.warranty}</span>
                </div>

                <div className="social-product-footer">
      ...
      <section id="inside" className="social-inside">
        <div className="social-inside-sticky">
          <p className="social-micro">03 / WHAT'S INSIDE</p>
          <h2>
            NO BLACK
            <br />
            BOX.
          ...
          <p>
            Mattress layers should not be mystery ingredients.
            Here is the construction story in plain language.
          </p>
        </div>

        <div className="social-layer-list">
          {(featured[0]?.layers || []).map((layer, index) => (
            <Reveal key={`${layer.name}-${index}`}>
              <article className="social-layer-card">
                <div className="social-layer-num">0{index + 1}</div>

                {layer.visualAsset ? (
                  <div className="social-layer-art">
                    <img src={layer.visualAsset} alt="" />
                  </div>
                ) : (
                  <GraphicPlaceholder label={`LAYER ${index + 1}`} />
                )}

                <div>
                  <p className="social-eyebrow">MATERIAL LAYER</p>
                  <h3>{layer.name}</h3>
